...
static void
emit_edge (int from, int to, const char *label)
{
  if (label && *label)
    {
...
static void
append_stmt_list_labels (ASTNode *node, char *buf, size_t bufsz)
{
  for (ASTNode * cur = node; cur; cur = cur->next)
    {
      if (strlen (buf) > bufsz - 8)
	break;
      switch (cur->type)
	{
	case NODE_DECLARATION:
	  strncat (buf, "decl; ", bufsz - strlen (buf) - 1);
	  break;
	case NODE_ASSIGNMENT:
	  strncat (buf, "assign; ", bufsz - strlen (buf) - 1);
	  break;
	case NODE_RETURN:
	  strncat (buf, "return; ", bufsz - strlen (buf) - 1);
	  break;
	case NODE_BREAK:
	  strncat (buf, "break; ", bufsz - strlen (buf) - 1);
	  break;
	case NODE_CONTINUE:
	  strncat (buf, "continue; ", bufsz - strlen (buf) - 1);
	  break;
	default:
	  strncat (buf, "stmt; ", bufsz - strlen (buf) - 1);
	  break;
	}
    }
}
...
static const char *
simple_stmt_label (ASTNode *node, char *tmp, size_t tmpsz)
{
  if (!node)
    return "";
  switch (node->type)
...
build_linear_stmt (ASTNode *stmt)
{
