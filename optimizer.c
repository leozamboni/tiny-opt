...
SymbolValue *evaluate_expression_value (ASTNode * expr, SymbolTable * table,
					char *scope);
void free_symbol_table (SymbolTable * table);

void
optimize_code (ASTNode *ast)
{
...
const char *
get_node_type_str (NodeType type)
{
  switch (type)
    {
    case NODE_PROGRAM:
...
void
print_dse_table (SymbolTable *head)
{
  printf
    ("\n=========================== DSE TABLE ===========================\n");
  printf ("%-3s %-15s %-15s %-10s %-15s %-15s %-15s %-15s %-10s\n", "#",
	  "Name", "Scope", "Type", "Value", "Loop Hash", "ID Hash",
	  "Node Type", "Dead?");
  printf
    ("------------------------------------------------------------------------------------------------------------------------------------\n");
  ...
  while (current != NULL)
	...
	dead_flag = current->node->is_dead_code;
      ...
      printf ("%-3d %-15s %-15s %-10s %-15s %-15" PRIu64 " %-15" PRIu64
	      " %-15s %-10s\n", index++,
	      current->name ? current->name : "(null)",
	      current->scope ? current->scope : "(null)", type_str, value_str,
	      current->loop_hash, current->id_hash,
	      current->node ? get_node_type_str (current->node->
						 type) : "(null)",
	      dead_flag ? "yes" : "no");
...
static int
is_pure_expression (ASTNode *expr)
{
  if (expr == NULL)
    return 1;
  switch (expr->type)
...
void
mark_subtree_dead (ASTNode *node)
{
  if (node == NULL)
    return;
...
int
eval_condition (ASTNode *condition, SymbolTable *head, char *scope)
{
  if (!condition)
    return -1;
...
void
reachability_analysis (ASTNode *node, SymbolTable *head, char *scope)
{
  if (node == NULL)
    return;
...
void
liveness_dse (SymbolTable *table)
{
	  ...
	  while (check)
	    {
		  ...
		  && strcmp (check->scope, entry->scope) == 0
...
SymbolValue *
resolve_final_value (SymbolTable *head, const char *name, const char *scope)
{
  SymbolTable *current = head;
...
SymbolValue *
evaluate_expression_value (ASTNode *expr, SymbolTable *table, char *scope)
{
  if (!expr)
    return NULL;
  ...
  switch (expr->type)
	...
	// CASO: ... && 0
	if (right_val && right_val->type == VALUE_TYPE_INT
	    && bin->op == OP_AND && right_val->number == 0)
	  {
	    return right_val;
	  }
	// CASO: 0 && ...
	...
	switch (bin->op)
	  ...
	  case OP_DIV:
	    result->number =
	      (right_val->number !=
	       0) ? left_val->number / right_val->number : 0;
	    break;
	    // case '%':
...
void
set_symtab (ASTNode *node, SymbolTable **head, SymbolTable **tail,
	    uint64_t loop_hash, char *scope)
{
  if (node == NULL)
    return;
  ...
  switch (node->type)
	...
	if (decl->name)
	  {
	    ...
	    *tail = new_entry;
	    ...
	    if (decl->initial_value)
	      {
		SymbolValue *val =
		  evaluate_expression_value (decl->initial_value, *head,
					     scope);
		if (val)
		  {
	...
	if (assign->variable)
	  {
	    ...
	    *tail = new_entry;
	    ...
	    if (assign->value)
	      {
		SymbolValue *val =
		  evaluate_expression_value (assign->value, *head, scope);
		if (val)
		  {
		    new_entry->value = malloc (sizeof (SymbolValue));
		    memcpy (new_entry->value, val, sizeof (SymbolValue));
    ...
    case NODE_WHILE_STATEMENT:
      {
	WhileNode *while_node = (WhileNode *) node;
	set_symtab (while_node->condition, head, tail, while_node->loop_hash,
		    scope);
	set_symtab (while_node->body, head, tail, while_node->loop_hash,
		    scope);
	break;
      }
    case NODE_FOR_STATEMENT:
	...
	set_symtab (for_node->condition, head, tail, for_node->loop_hash,
		    scope);
	set_symtab (for_node->increment, head, tail, for_node->loop_hash,
		    scope);
...
int
all_statements_dead (ASTNode *stmt_list)
{
...
int
has_return_statement (ASTNode *node)
{
  if (node == NULL)
    return 0;
...
int
has_break_continue (ASTNode *node)
{
  if (node == NULL)
    return 0;
...
void
free_symbol_table (SymbolTable *table)
{
  ...
  while (current)
    {
      ...
      if (current->value)
	{
	  if (current->value->type == VALUE_TYPE_STRING
	      && current->value->str)
	    {
...
void
print_optimization_report (ASTNode *ast)
{
